...
- Outcome: found multiple absolute-path occurrences in `training/` backup files and in some `userData` backups; found one email (`john@example.com`) inside `training/training.jsonl` (user confirmed this is an acceptable copyright contact).
...
- Files added:
  - `scripts/precommit_check_paths.js` — scans staged files (using `git diff --cached --name-only`) and `git show :path` for staged contents, searches for Windows absolute paths (`C:\\...`) and UNC (`\\\\host\\share\\...`) and blocks commit when found.
