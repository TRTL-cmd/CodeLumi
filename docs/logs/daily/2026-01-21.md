````markdown
2026-01-21 — Daily Engineering Log
=================================
...
1) Speech bubble behavior and UX
  - What I changed:
    ...
    - Ensured the bubble supports inline feedback controls (👍 👎 Teach) and wired those controls to `recordFeedback`, `showDialog()` for corrections, and `addOrUpdateKB()` for teaching.
  - Why:
...
4) Code editor toggle and Say Hi button
  - What I changed:
    ...
    - Added a click handler for `#speakBtn` (Say Hi) that calls `say('Hi there — what can I do for you?')` so the quick greeting reliably shows.
  - Why:
...
-------------------------------------------------------------
Priority 1 — Stream parsing & normalization (blocker)
...
Notes and rationale
-------------------
- The changes focused on UX and making tools accessible (history, teach, repeatable interactions) while preserving 3D animation behavior and LLM integrations. The remaining critical engineering work is stream parsing of the Ollama output — once done, the assistant will present coherent replies and the history/feedback flows will be fully useful.

Where I can help next (pick one):
- I can implement stream parsing now inside `src/core/llm/ollama.ts` and update the Brain wrapper; this is the cleanest long-term fix.
- Or I can apply a quick normalization hotfix to `dist-electron/main.js` to unblock dev and packaged behavior immediately.

— End of 2026-01-21 log
